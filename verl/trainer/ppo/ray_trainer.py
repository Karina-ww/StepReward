...
class RayPPOTrainer:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        if self.config.algorithm.get("use_step_prob_reward", False):
                            ...
                            # 把概率增量回填为 token-level scores，继续走原 KL/adv 流程
    ...
    def compute_step_prob_rewards(self,
        ...
        cut_counts: list[list[int]] = []
        for i in range(B):
            # peaks = self._pick_topk_peaks_with_gap(
        ...
        # V: [B*(k+1)]
        ##########debug##########
        # 提取 pr_batch 中的 responses 和 ground_truth_mask
        pr_responses = pr_batch.batch['responses']  # 形状 [B*(k+1), R]，响应部分token ids
        pr_gt_mask = pr_batch.batch['ground_truth_mask']  # 形状 [B*(k+1), R]，1表示GT位置
        B_k = pr_responses.size(0)  # B*(k+1)

        gt_mask_sum = pr_gt_mask.sum().item()
        gt_mask_per_sample = pr_gt_mask.sum(-1).tolist()
        print(f"pr_gt_mask 总共有 {gt_mask_sum} 个1")
        print(f"每个样本的gt_mask中1的数量: {gt_mask_per_sample[:5]}...")  # 前5个样本

        log_probs_sum = log_probs.sum().item()
        log_probs_gt_sum = (log_probs * gt_mask).sum().item()
        print(f"log_probs 总和: {log_probs_sum}（全0则异常）")
        print(f"log_probs 在gt_mask为1的位置总和: {log_probs_gt_sum}（全0则V必为0）")
        sample_indices = range(min(3, B_k))
        for idx in sample_indices:
        
            resp_ids = pr_responses[idx]  # [R]
            mask = pr_gt_mask[idx]  # [R]，1表示GT位置
            
            gt_token_ids = resp_ids[mask == 1]  # 形状 [M]，M为GT的token数量
            
            # 解码GT token ids
            decoded_gt = self.tokenizer.decode(gt_token_ids, skip_special_tokens=True)
            
            # 对应原始的ground truth（需映射回原始B个样本，因为pr_batch是B*(k+1)）
            original_idx = idx // (k + 1)  # 每个原始样本对应(k+1)个pr样本
            original_gt = ground_truth_list[original_idx]
            
            # 打印对比结果
            print(f"\n===== pr_batch样本 {idx}（对应原始样本 {original_idx}） =====")
            print(f"gt_mask标记的解码结果: {decoded_gt}")
            print(f"原始ground truth: {original_gt}")
            print(f"是否匹配: {decoded_gt == original_gt}")
        ##########debug##########

        V = (log_probs * gt_mask).sum(-1) / (gt_mask.sum(-1) + 1e-8)
        print("===V===", V)
        # 把 V 拆回每个样本的 [k+1] 列表
        Vs: list[list[float]] = []
        idx = 0
        ...
        # 4) 回填 token-level scores 到原 batch.input_ids 对齐的二维张量
    ...
    def replace_answer_with_gt_batch(
        ...
        for i in range(batch_size):
            ...
            # Truncate / pad
            prompts = F.pad(prompts, (max_length - prompts.shape[0], 0), value=self.tokenizer.pad_token_id)
            responses = F.pad(responses, (0, self.config.data.max_response_length - responses.shape[0]), value=self.tokenizer.pad_token_id)

            full_input_ids = torch.cat([prompts, responses], dim=0)
            full_attention_mask = F.pad(attn_mask, (0, full_input_ids.shape[0] - attn_mask.shape[0]), value=0)
