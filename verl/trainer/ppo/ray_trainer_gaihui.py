...
class RayPPOTrainer(object):
    ...
    def __init__(self,
        ...
        elif self.config.algorithm.adv_estimator in ['grpo', 'grpo_nostd', 'reinforce_plue_plus', 'remax', 'rloo', 'prnostd_vrstd', 'prstd_vrstd', 'seg_grpo']: ### WWW:add
    ...
    def _maybe_log_train_generations_to_wandb(self, sequences, advantages, scores, scoreAs, scoreBs, 
                                              format_rewards,  entropies, data_sources, extracted_answers, ground_truths,
                                              vr_scores=None, pr_scoreAs=None, pr_scoreBs=None,final_reward_stds=None,
            ...
            while True:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        if self.config.reward_model.get('manager', '') == 'prob_step:
                            ground_truth_list = [
                                batch[i_].non_tensor_batch['reward_model']['ground_truth']
                                for i_ in range(len(batch))
                            ]
                            ground_truth_list = [
                                item for item in ground_truth_list
                                for _ in range(self.config.actor_rollout_ref.rollout.n)
                            ]

                            k = self.config.reward_model.get('step_k', 4)
                            min_gap = self.config.reward_model.get('step_min_gap', 50)

                            token_level_scores = self.compute_step_prob_rewards(
                                batch=batch,
                                ...
                                k=k,
                                min_gap=min_gap,
                                assign_mode=self.config.reward_model.get('step_assign_mode',
                                                                          'segment_last_token')
                            )
                            ...
                            # 记录 step 奖励分布
                            step_reward_sample = token_level_scores.sum(-1)
                            metrics.update({
                                'critic/step_reward/mean': step_reward_sample.mean().item(),
                                'critic/step_reward/max' : step_reward_sample.max().item(),
                                'critic/step_reward/min' : step_reward_sample.min().item(),
                            })
                        elif self.config.reward_model.get('manager', '') != 'prob_step':
    ...
    def _pick_topk_peaks_with_gap(self,
        ...
        """
        在 response 熵向量里挑前 k 个局部峰，保持最小间隔，返回升序索引。
        """
        L = int(valid_len)
        scores = entropy_vec[:L].cpu().tolist()
        selected, suppressed = [], [False] * L
        for _ in range(k):
            best = max([(s, i) for i, s in enumerate(scores) if not suppressed[i]], default=(None, None))
            if best[0] is None:
                break
            idx = best[1]
            selected.append(idx)
            for j in range(max(0, idx - min_gap), min(L, idx + min_gap + 1)):
                suppressed[j] = True
    ...
    def _build_context_gt_batch(self,
        ...
        利用已有 replace_answer_with_gt_batch 以最小改动得到 DataProto。
        """
        prompts_ids = gen_batch_output.batch['prompts']         # (B,P)
        responses_ids = gen_batch_output.batch['responses']     # (B,R)
        B = prompts_ids.size(0)

        # 拆分后逐样本组装，为复用你的批处理函数，这里直接收集到 lists
        merged_prompts, merged_responses = [], []
        merged_input_ids, merged_attn, merged_pos, merged_gt_mask = [], [], [], []
        merged_uids, batch_rows = [], []

        for i in range(B):
            uid_i = gen_batch_output[i].non_tensor_batch['uid']
            prompt_ids = prompts_ids[i]
            resp_ids   = responses_ids[i]
            for c in cut_counts[i]:
                resp_prefix = resp_ids[:c]
                merged_prompts.append(prompt_ids.clone())
                merged_responses.append(resp_prefix.clone())
                merged_uids.append(uid_i)

        # 交由原函数批处理（传参 suffix 让 key 不冲突）
        stub_batch = DataProto.from_single_dict({
            'prompts': torch.stack(merged_prompts),
            'responses': torch.stack(merged_responses),
            'input_ids': torch.stack(merged_prompts),  # placeholder，后面会被 replace 内部覆盖
        })
        max_length = self.config.data.max_prompt_length + self.config.data.max_response_length
        pr_batch = self.replace_answer_with_gt_batch(
            gen_ids_batch=stub_batch.batch['input_ids'],   # dummy,未用
            gen_response_batch=stub_batch.batch['responses'],
            ground_truth_batch=ground_truth_list * (len(cut_counts[0])),
            ...
            start_answer=start_answer, end_answer=end_answer,
            max_length=max_length,
            suffix='_step'  # 用默认字段名
        )
        pr_batch.non_tensor_batch['uid'] = np.array(merged_uids, dtype=object)
        return pr_batch
    ...
    def compute_step_prob_rewards(self,
        ...
        """
        返回与 batch.input_ids 对齐的 token_level_scores (float32)
        """
        ...
        valid_lens = attn_resp.sum(-1).tolist()

        # 1) 选切点
        cut_counts = []
        for i in range(B):
            peaks = self._pick_topk_peaks_with_gap(response_entropy[i],
                                                int(valid_lens[i]),
                                                k, min_gap)
            cut_counts.append([0] + [p + 1 for p in peaks])  # 前缀 token 数

        # 2) 构造 (B*(k+1)) GT 批次
        pr_batch = self._build_context_gt_batch(gen_batch_output,
                                                ground_truth_list,
                                                cut_counts)

        # 3) 一次前向拿 Vj
        log_probs = self.actor_rollout_wg.compute_log_prob(pr_batch)['old_log_probs'].batch
        Vs, idx = [], 0
        for i in range(B):
            cur = []
            for _ in cut_counts[i]:
                m = pr_batch[idx].batch['ground_truth_mask'].bool()
                cur.append(log_probs[idx][m].mean().item())
                idx += 1
            Vs.append(cur)

        # 4) 组装 token_level_scores
        scores = torch.zeros_like(batch.batch['input_ids'], dtype=torch.float32)
        for i in range(B):
            rs = [Vs[i][j] - Vs[i][j-1] for j in range(1, len(Vs[i]))]  # 长度 k
            seg_ends = [c-1 for c in cut_counts[i][1:]]  # response 内 token idx
            prev = 0
            for r, end_idx in zip(rs, seg_ends):
                if assign_mode == 'segment_last_token':
                    scores[i, prompt_len + end_idx] = r
                else:  # uniform
                    start = prompt_len + prev
                    end   = prompt_len + end_idx + 1
                    length = max(1, end - start)
                    scores[i, start:end] = r / length
                prev = end_idx + 1
        return scores
